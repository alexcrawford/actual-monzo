...
export const AccountMappingSchema = z.object({
  monzoAccountId: z.string()
    .regex(/^acc_[a-zA-Z0-9]{16,}$/, 'Monzo account ID must match format acc_XXXXXXXX...'),

  monzoAccountName: z.string()
    .min(1, 'Monzo account name cannot be empty'),

  actualAccountId: z.string()
    .uuid('Actual Budget account ID must be valid UUID'),

  actualAccountName: z.string()
    .min(1, 'Actual Budget account name cannot be empty')
});
...
export const MonzoConfigSchema = z.object({
  clientId: z.string()
